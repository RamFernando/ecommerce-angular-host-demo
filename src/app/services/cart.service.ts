...
export class CartService {
  ...
  ServerURL = 'http://localhost:8084/services/';
  ...
  //Obervables for the components to subscribe
  ...
  AddProductToCart(id: Number, quantity?: number) {
    ...
    this.productService.getSingleProduct(id).subscribe(prod => {
      ...
      // Cart is not empty
        ...
        let index = this.cartDataServer.data.findIndex(p => p.product.id === prod.id); //-1 or positive val

        // 1. If chosen product is already in cart array => positive val
        if (index !== -1) {

          if (quantity !== undefined && quantity <= prod.qty) {
            // @ts-ignore
            this.cartDataServer.data[index].numInCart = this.cartDataServer.data[index].numInCart < prod.qty ? quantity : prod.qty;
          } else {
            // @ts-ignore
            // console.log("else");
            this.cartDataServer.data[index].numInCart < prod.qty ? this.cartDataServer.data[index].numInCart++ : prod.qty;
  ...
  UpdateCartData(index, increase: Boolean) {
    ...
    if (increase) {
      // @ts-ignore
      data.numInCart < data.product.qty ? data.numInCart++ : data.product.qty;
  ...
  CheckoutFromCart(userId: Number) {
    ...
    // this.httpClient.post(`${this.ServerURL}orders/payment`, null).subscribe((res: { success: Boolean }) => {
      ...
      // if (res.success) {
        ...
        // this.httpClient.post(`${this.ServerURL}orders/add`, {
        //   userId: userId,
        //   products: this.cartDataClient.prodData
        // }).subscribe((data: OrderConfirmationResponse) => {

    
          // this.orderService.getSingleOrder(1).then(prods => {
